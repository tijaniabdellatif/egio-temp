function MLSelectInput(element,attempts = 5) {

    if (attempts == 0) return;

    if(!element) {
        setTimeout(() => {
            MLSelectInput(element,attempts - 1);
        }, 1000);
        return;
    }
...
// find ml-select-input
(()=>{
    let elements = document.querySelectorAll('.ml-select-input');
    elements.forEach(element => {
        MLSelectInput(element);
    });
})();
