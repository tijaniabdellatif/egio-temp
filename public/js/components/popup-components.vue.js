let PopupComponent = {
    /*html*/
    template: `
        <div class="popup" :class="display?'show':''" >
    ...
    watch: {
        display: function (val) {
            ...
            //to scroll to the top of popup body
            document.querySelector(".popup-body").scrollTo(0, 0);
        },
    ...
    methods: {
        close() {
            ...
            swal({
                title: "Êtes-vous sûr?",
                text: "Vous ne pourrez pas retourner cela!",
                icon: "warning",
                buttons: true,
                dangerMode: true,
            }).then((close) => {
...
let PopsupComponent = {
    template: `
    <div class="s_popup_cnt" :class="display?'show':''" @click.self="close()">
        <div class="s_popup" >
        <div class="bg"></div>
            <div class="s_popup-header">
                <div class="s_popup-title">
                    <span>{{ title }}</span>
                    <div class="spinner-border spinner-border-sm ms-2" v-if="loading" role="status">
                        <span class="sr-only">Loading...</span>
                    </div>
                </div>
                <button class="s_popup-close-btn" @click="close()">
                    <i class="fa-solid fa-xmark"></i>
                </button>
            </div>
            <div class="s_popup-body">
                <div class="s_popup-container">
                    <slot></slot>
                    <div v-if="loading" class="loading-container" >
                        <!--<div class="spinner-border spinner-border-sm ms-2" v-if="loading" role="status">
                            <span class="sr-only">Loading...</span>
...
`,
    ...
    watch: {
        display: function (val) {
            ...
            //to scroll to the top of popup body
            document.querySelector(".s_popup-body").scrollTo(0, 0);
        },
