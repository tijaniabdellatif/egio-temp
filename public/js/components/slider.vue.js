const Slider = {
    ...
    template: `
        <div class="images_slider">
            ...
            <img v-if="images.length == 0" class="show" src="/assets/img/no-photo.png" />
            <img v-else :src="toThumbnail('/storage'+images[index].name)" />
        </div>
    `,
    props: {
        images:[]
    ...
    methods: {
        ...
        toThumbnail(src){
            return src.replace('/storage/images/', '/storage/images/tn_').replace('/storage/old/', '/storage/old/tn_').replace('/storage/flux/', '/storage/flux/tn_');
        }
    },
};
